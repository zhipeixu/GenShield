...
class AnomalyRepairStage2IterableDataset(DistributedIterableDataset):
    """
    Dataset for image anomaly repair task - Stage 2 (Interleaved Text + Image Generation).
    
    Stage 2 Training Flow:
    Input:
        1. Anomalous image (VIT + VAE encoding)
        2. Short system prompt (no anomaly description)
    
    Output (with loss):
        1. Anomaly description text (CE loss)
        2. Repaired image (MSE loss)
    
    This enables the model to:
        - First analyze and describe the anomalies
        - Then generate the repaired image
    ...
    def __init__(
        ...
        **kwargs  # Accept any additional unexpected parameters
        ...
        # Stage 2 system prompt - shorter, no anomaly description in input
    ...
    def parse_line(self, line):
        """
        Parse a single line from the JSONL file for Stage 2.
        
        Stage 2 Sequence:
        [Input - No Loss]
        1. Anomalous image (VIT + VAE)
        2. Short system prompt
        
        [Output - With Loss]
        3. Description prefix + Anomaly description + Repair trigger (CE loss)
        4. Repaired image (MSE loss)
        ...
        # Step 2: Add the short system prompt (no loss, no caption)
        data = self._add_text(data, self.system_prompt, need_loss=False)
        
        # ============ OUTPUT SECTION (With Loss) ============
        
        # Step 3: Add the anomaly description output (CE loss)
        # This is what the model should learn to generate
        # Format: description_prefix + caption + repair_trigger
        output_text = self.description_prefix + caption + self.repair_trigger
        data = self._add_text(data, output_text, need_loss=True)  # CE loss on this text
