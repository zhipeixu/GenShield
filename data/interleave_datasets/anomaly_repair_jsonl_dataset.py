...
class AnomalyRepairJSONLIterableDataset(DistributedIterableDataset):
    """
    Dataset for image anomaly repair task - JSONL version.
    ...
    The model takes:
    1. Anomalous image (before_edit) - as VIT + VAE input
    2. System prompt + caption + repair trigger
    
    And outputs:
    3. Repaired image (after_edit) - with MSE loss applied
    """
    ...
    def __init__(
        ...
        system_prompt=None,
        repair_trigger=None,
        data_dir_list=None,  # Added to accept but ignore this parameter from dataset_base
        **kwargs  # Accept any additional unexpected parameters
        ...
        if system_prompt is None:
            self.system_prompt = (
                "You are an image anomaly repair assistant. This is an AI-generated image, but it has some defects and anomalies. Please fix these abnormal areas so that it looks as realistic and natural as possible. The following are some areas where problems exist:\n\n"
            )
        else:
            self.system_prompt = system_prompt
        
        # Default repair trigger
        if repair_trigger is None:
            self.repair_trigger = (
                "\n\n<repair>Check the abnormal area in the image and generate the image with the abnormality repaired.</repair>"
            )
        else:
    ...
    def parse_line(self, line):
        ...
        # Step 2: Add the text prompt (system + caption + trigger)
        full_prompt = self.system_prompt + caption + self.repair_trigger
        data = self._add_text(data, full_prompt, need_loss=False)
        
        # Step 3: Add the repaired image (output, with loss)
